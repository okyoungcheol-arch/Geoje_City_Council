...
export function parseMinutesHtml(html: string): ScrapedStatement[] {
  ...
  // every turn in it would leak through silently as a normal statement. Cross-check against
  // `#item-block`, the document's independent TOC of agenda items (rendered separately from
  // the in-body headers, e.g. `<a href="#item1" title="○5분 자유발언(...)">`): if the TOC
  // lists a free-speech agenda item but no in-body header was ever detected as matching it,
  // refuse to guess — throw instead of returning statements that might include free speeches.
  // Known residual gap (accepted, not fixed): this is a document-wide boolean, so a header
  // that appears textually *after* its own turns would still set the flag and suppress the
  // throw — it does not protect against out-of-order emission, only against an omitted or
  // differently-shaped header. Not observed in any real document to date.
