...
이 시스템에는 사람 코딩 에이전트(Claude Code)와는 별개로, **런타임에 실제로 데이터를 처리하는 두 개의 LLM 에이전트**가 있습니다. 이 문서는 그 둘의 역할, 입출력 계약, 프롬프트/루브릭을 정의합니다. 구현은 `backend/lib/ai/summarize.ts`, `backend/lib/ai/extractQaRounds.ts`, `backend/lib/ai/matchIssues.ts`, `backend/lib/scoring/kpi.ts`, 오케스트레이션은 `backend/lib/pipeline/processStatement.ts`를 참고하세요. 루브릭 원문은 `docs/rubric/CLAUDE.md`(v2.0)입니다.
...
- **출력 스키마:**
  ...
  interface SummaryResult {
    ...
    citations: Citation[];            // { type: "L"|"S"|"P"|"F"; text: string }[] — KPI1 근거밀도용
    proposals: Proposal[];            // { budget, timeline, subject, method: boolean }[] — KPI2 대안구체성용
    selfRaisedIssues: SelfRaisedIssue[]; // { description: string }[] — KPI5 이슈지속추적률용
  }
  ```
...
- **출력 스키마:**
  ...
  interface QaRound {
    ...
    bonusTags: string[]; // "모순포착" | "패턴제시" | "회피차단" | "법근거제시"
  }
  ```
- **역할:** 왕복(round)마다 답변등급과 가산 태그를 분류한다 — KPI3(추궁심도)·KPI4(답변확보율)의 원재료. 질의응답 구조가 없으면(`hasQaStructure`가 false) 호출 자체를 생략하고 두 KPI는 N/A가 된다.
...
`backend/lib/scoring/kpi.ts`가 Agent 1/1b의 추출 결과로부터 KPI1~4를 계산하고, KPI5(이슈지속추적률)는 `issueTickets`/`issueReviews` 누적치로 쿼리 레이어(`backend/lib/queries/insights.ts`)에서 계산한다. 산식·등급 경계값·N/A 조건은 `docs/rubric/CLAUDE.md` §3이 단일 진실 소스다.
...
- 루브릭 문구를 수정하면 기존에 처리된 `statement_insights` 행과 채점 기준이 달라진다. `rubricVersion` 컬럼(현재 `"v2.0-5kpi"`)으로 버전을 구분하되, 루브릭을 바꿀 때는 영향받는 행을 지우고 재실행하는 것을 고려하라.
- 모든 모델 호출은 `generateObject` + zod 스키마로 구조화 출력을 강제한다. 스키마를 벗어난 응답은 SDK가 에러를 던지므로, 파이프라인의 재시도 로직이 이를 처리한다.
- KPI1(근거밀도)은 발언시간(초) 데이터가 있어야 계산되는데, 스크래퍼(`backend/scripts/scrape/minutes.ts`)가 아직 이 값을 수집하지 않아 현재는 항상 N/A다 — `processStatement.ts`의 TODO 참조.
